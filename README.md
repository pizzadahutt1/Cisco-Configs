...
The copy command will ask you a few things, like the IP of your TFTP server and the file name you want to save it as

The syntax of the copy command is that you are copying from "here" to "there". I.E. "copy fromhere: tothere:". You can even add a specific filename in there if you already know that, like "copy tftp:file1 flash:file1". If you don't, it will ask you what the file name is you want to move and what to save it as. Want to copy the file but save it as a different name? "copy tftp:file1 flash:newfilename". All storage needs to have a colon at the end, exceptions are the running-config and startup-config. running-config is what you are writing to every time you make a configuration change. statup-config is what you are saving the running-config as every time you issue a write or write memory command. The startup-config is saved in the NVRAM, while the running-config is kept in ram. However, in our case, the NVRAM is doinked so we cannot write to the startup-config. 
